...
Windows users will probably need Rtools installed first
...
listening posts, with a distance of 1 km between adjacent posts.
...
plots of the covariates using the following R code:

<<plot-covariates, eval = FALSE>>=
plotcov(cov.df, "x")
@

Replace \texttt{"x"} with the name of a different column in
\texttt{cov.df} to take a look at different variables.
...
your system and upload it using this linke:
