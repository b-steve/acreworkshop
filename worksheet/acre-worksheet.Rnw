...
install these packages is to first install the \texttt{devtools}
...
<<package-load, eval = FALSE>>=
library(acre)
library(acreworkshop)
@ 

Also, a quick disclaimer: the \texttt{acre} package is still under
...
(Figure \ref{fig:ppws-outline}). Exactly why they have appointed
...
<<ppws-outline, fig.height = 4, echo = FALSE, fig.cap = "A map of Phnom Prich Wildlife Sanctuary. The x- and y-coordinates are in metres.">>=
...
{\it Nomascus gabriallae} is currently listed as an endangered species
...
live?\footnote{Note that the data you will be using in this exercise
  ...
  the density and distribution of {\it Nomascus gabriallae} in Phnom
  Prich Wildlife Sanctuary, but the challenges you will face here are
  similar to those from a real data set.} To achieve this goal, you
will conduct an acoustic survey: you will select 18 locations
throughout the park, and at each, your team will establish three
...
\citet{Kidney2016} and \citet{McGrath2023}, for example. In this
...
region\footnote{This sentence isn't quite true in reality, but let's
...
\texttt{"var"} with the name of a column in \texttt{cov.df} to take a
...
in \texttt{villages.df}, you have listening post locations in
...
an R object using the \texttt{read.acre()} function. It is your job to
...
\item \texttt{control.mask}: you need to specify the maximum distance
  at which you can possibly detect a gibbon. In this case, the maximum
  feasible distance is $3\,000$ m. Use \texttt{control.mask =
    list(buffer = 3000)} for this argument.
...
\item \texttt{dist.cov}: a data frame containing locations of objects
  ...
  point in PPWS, so you can use \texttt{dist.cov = list(village =
    villages.df)}.
\end{enumerate}
...
First, the detection function can be selected with the \texttt{detfn}
argument. The default is \texttt{detfn = "hn"}, corresponding to the
halfnormal detection function. To adjust the detection function, you
can set the \texttt{detfn} argument to \texttt{"hhn"} for the hazard
halfnormal function or \texttt{"th"} for the threshold detection
function.
...
your system and upload it using this link:
\url{https://drive.google.com/drive/folders/1SghP6bAKhMHvhIHC-c17r-RACmflBjOT?usp=sharing}. If
you're not sure what your R working directory is, then run
\texttt{getwd()} to find out.

\section*{In case you're interested...}

The density of southern yellow-cheeked crested gibbon ({\it Nomascus
  gabriallae}) groups in this simulated example were rather higher
...
of the closely related northern yellow-cheeked crested gibbon ({\it
